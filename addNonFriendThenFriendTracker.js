/**
 * addNonFriendThenFriendTracker script
 *
 * Profile-wise backfill:
 * - Finds `friend_lists` docs where:
 *   - last_non_friend_at exists (non-null)
 *   - friendStatus = "Activate"
 *   - deleted_status = 0
 * - Inserts activity records:
 *   1) "Non friend" added at created_at = last_non_friend_at
 *      - friendStatusChanged (17) + DOB (5) + Gender (11) + Country (12)
 *      - Only these fields are tracked for the non-friend event
 *   2) "Non friend" -> "Friend" at created_at = friend_lists.created_at
    ...
    try {
        ...
        for (const profile of profiles) {
            ...
            while (await cursor.hasNext()) {
                ...
                // 1) Friend -> Non friend at last_non_friend_at
                batch.push({
                    user_id,
                    fb_user_id,
                ...
                // While adding as Non friend: only add DOB, gender, country with script flag
            ...
            if (profileInserted > 0) {
                console.log(
                    `Profile user_id=${user_id}: inserted ${profileInserted} non-friend + non-friend->friend activity records.`
                );
            }
        }

        console.log(
            `Done. Total non-friend + non-friend->friend activity records inserted: ${totalInserted}.`
        );
    } catch (error) {
