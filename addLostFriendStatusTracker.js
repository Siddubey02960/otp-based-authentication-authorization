/**
 * addLostFriendStatusTracker script
 *
 * Condition:
 * - friendStatus: "Lost"
 *
 * Backfill (when last_non_friend_at exists, meaning Non friend -> Friend -> Lost):
 ...
 * 3) Add transition "Friend" -> "Lost" at created_at = friend_lists.lost_at
 *
        ...
        for (const profile of profiles) {
            ...
            while (await cursor.hasNext()) {
                ...
                if (isNonEmpty(nonFriendAt)) {
                    ...
                    // 2) Non friend -> Friend at last_friend_at (fallback: created_at)
                    batch.push({
                        user_id,
                        fb_user_id,
                        created_at: friendAt,
                    ...
                    // Start with Friend if last_non_friend_at is not present
                    batch.push({
                        user_id,
                        fb_user_id,
                        created_at: friendAt,
                ...
                batch.push({
                    ...
                    script9: true ,
                    friendFbId,
                    activity_info: { oldValue: 'Friend', newValue: 'Lost' },
                });
            ...
            if (profileInserted > 0) {
                console.log(
                    `Profile user_id=${user_id}: inserted ${profileInserted} lost-friend activity records.`
                );
            }
        }

        console.log(
            `Done. Total lost-friend activity records inserted: ${totalInserted}.`
        );
    } catch (error) {
